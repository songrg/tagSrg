...
void cWall::Setup(int iRow, int iCol, float fSize)
{
	m_iRow = iRow;
	m_iCol = iCol;
	m_fSize = fSize;
	...
	for (int z = 0; z <= m_iRow; z++)		// 행 = z축
	{
		...
		{
			// 버텍스 지정
			m_pGridVertex[vIndex].p.x = 30.0f;
			m_pGridVertex[vIndex].p.y = m_fStartPosX + (x * m_fSize);
...
void cWall::Render(bool isShowWire)
{
